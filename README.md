...
Os LEDs vermelho, amarelo e verde irão ser ligados respetivamente aos pinos 21,22 e 19 da placa ESP32. Cada LED é ativo a nível alto devido a uma diferença de tensão que irá provocar uma corrente que atravessa o LED. Cada LED está protegido por uma resistência de 1k $\Omega$ de modo a não queimar. Os botões irão ser ligados ao pinos 23 (botão esquerdo) e 18 (botão direito). Os botões são ativos ao nível baixo, ou seja, quando o botão é premido, o pino da placa ESP32 ligado ao botão assume o valor 0. Os botões estão protegidos por resistências de 1k&Omega tal como os LEDs, sendo que para cada botão existe um condensador de 100nF em paralelo com a resistência como mecanismo de anti-bouncing para o botão.
